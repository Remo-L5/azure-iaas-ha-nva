...
# Gateway Load Balancer for traffic inspection via VXLAN tunnels
module "gwlb" {
  ...
  location            = var.location
  name                = local.svc_config.gateway_load_balancer_name
  ...
  frontend_subnet_resource_id = local.svc_config.subnet_resource_id

  frontend_ip_configurations = {
    gwlb_frontend = {
      name                                   = local.gwlb_fe_config_name
  ...
  backend_address_pools = {
    gwlb_pool = {
      name = local.gwlb_be_pool_name
      tunnel_interfaces = {
        internal_tunnel = {
          identifier = local.svc_config.internal_tunnel_identifier
          type       = "Internal"
          protocol   = "VXLAN"
          port       = local.svc_config.internal_tunnel_port
        }
        external_tunnel = {
          identifier = local.svc_config.external_tunnel_identifier
          type       = "External"
          protocol   = "VXLAN"
          port       = local.svc_config.external_tunnel_port
        }
      }
    }
  }
  ...
  lb_rules = {
    gwlb_rule = {
      name                              = "rule-gwlb-ha-ports"
      frontend_ip_configuration_name    = local.gwlb_fe_config_name
      backend_address_pool_object_names = ["gwlb_pool"]
      probe_object_name                 = "gwlb_probe"
      protocol                          = "All"
      frontend_port                     = 0
      backend_port                      = 0
...
module "slb_service_chain" {
  ...
  frontend_ip_configurations = local.svc_enabled ? {
    service_chain = merge(
      ...
      {
        gateway_load_balancer_frontend_ip_configuration_resource_id = module.gwlb[0].azurerm_lb.frontend_ip_configuration[0].id
      }
    )
  } : {}
      ...
      service_chain_network_interface = {
        ...
        ip_configurations = {
          service_chain_ip_config = {
            ...
            load_balancer_backend_pools = {
              gwlb_pool = {
                load_balancer_backend_pool_resource_id = module.gwlb[0].azurerm_lb_backend_address_pool["gwlb_pool"].id
              }
            }
          }
        }
