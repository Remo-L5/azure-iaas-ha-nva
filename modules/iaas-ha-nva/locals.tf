locals {
  ...
  svc_config = local.svc_enabled ? {
    ...
    gateway_load_balancer_frontend_ip_configuration_id = var.service_chain_configuration.gateway_load_balancer_frontend_ip_configuration_id
    probe_protocol                                     = coalesce(try(var.service_chain_configuration.probe_protocol, null), "Tcp")
    probe_port                                         = coalesce(try(var.service_chain_configuration.probe_port, null), 80)
